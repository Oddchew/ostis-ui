...
std::string HTTPRequestHandler::GetFileMimetype(ScMemoryContext *context,
                                                ScAddr const &scFile) {
  ...
  FileFormatHTTPDescription.Quintuple(
      ...
      ScType::VarNodeClass >> FileFormatClassAlias, ScType::VarPermPosArc,
...
void HTTPRequestHandler::FileRetriever(ScMemoryContext *context,
                                       ScAddr &fileAddr,
  ...
  try {
    std::string fileContent;
    bool const getContentResult =
        context->GetLinkContent(fileAddr, fileContent);
    if (!getContentResult) {
...
void HTTPRequestHandler::RetrieveCurrentUIHandler(httplib::Request const &req,
                                                  httplib::Response &res) {
  // TODO: use user-specific current model
  ScAgentContext *context = new ScAgentContext();

  ScAddr const currentModel = utils::IteratorUtils::getAnyFromSet(
      context, HTMLTranslatorKeynodes::concept_current_ostis_ui_model);
...
void HTTPRequestHandler::FileByAddrRequestHandler(
    const httplib::Request &req, httplib::Response &res) noexcept {
...
void HTTPRequestHandler::FileBySystemIdtfRequestHandler(
    const httplib::Request &req, httplib::Response &res) noexcept {
  ScMemoryContext *context = new ScMemoryContext();
  ScAddr fileAddr;
